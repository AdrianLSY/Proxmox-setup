...
The VM is configured with `--onboot 1` which ensures it automatically starts when the Proxmox host boots (or reboots after an unexpected shutdown). This is critical for a router/firewall VM to maintain network connectivity. The hook script guarantees CPU pinning is applied during the automatic startup.
